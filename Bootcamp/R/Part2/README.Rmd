...
NE.health_subset1 <- filter ( NE.health_2, p.smokers > 15 )
...
#### **Exercise**
In *demogr2* data leave only those observations where *Population* is greater than 50K and 
household income (*house_inc* ) is less than 75K. Store the result in a data frame
called demogr_sub.

```{r, echo=FALSE}
demogr_sub <- filter ( demogr2, Population > 50000 &  house_inc < 75000 )
```
...
NE.health_3 <- mutate ( NE.health_2, both = (p.smokers > 15 & p.obese > 30) )
head(NE.health_3)
```
...
NE.health_4 <- arrange ( NE.health_3, p.smokers )
head(NE.health_4)

# Sort by percent of obese adults in reverse order
NE.health_5 <- arrange ( NE.health_3, desc(p.obese) )
head(NE.health_5)
```
