...
ggplot( NE.data, aes (x = house_income) ) + geom_histogram(  binwidth = 10000 )
ggplot( NE.data, aes (x = p_housing_problems) ) + geom_histogram(binwidth = 1 )
...
ggplot( NE.data, aes (x = State, y = life_exp) ) + 
  geom_boxplot() + 
  geom_jitter( width = .2)
...
ggplot( NE.data, aes(x = ph_distress, y = life_exp)) + 
  geom_point() +
  geom_smooth ( method = lm )
...
ggplot( USA.data, aes(x = ph_distress, y = life_exp)) + 
  geom_point(alpha = 0.1) + 
  geom_smooth(method="lm")+
  labs( title = "Life Expectancy vs. Physical Distress",
        x =  "Physical Distress (%)",  
