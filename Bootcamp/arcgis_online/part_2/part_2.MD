...
2. Click on `Content` --> `My Content`
3. Click on `Web Map: Demographics - <username>` web map you created in Part 1.
4. Click on `Open in Map Viewer` button in the top left.
...
2. Click on the `Show Table` icon to make the attribute table visible.
3. While still hovering the mouse over the layer, click on the three dots `...` and click on `Configure Pop-up`
...
4. Enter the following line into the `Pop-up Title` field:
> County: {COUNTY} - {STATE}

<img src="snippets/2_map_layer_pop_up_title_change.png" width="400">

5. Now click `OK` at the bottom of the form.
6. Cover your mouse over a county in the map viewer and click on the county to generate the pop-up.
...
7. Hover the mouse over the layer `% Frequent Physical Distress`.
8. Click on the three dots `...` and then `Configure Pop-up`
...
9. Click on `Configure Attributes` link at the bottom of the section.
...
10. Top left, click on the `Display` checkbox and then click it again to uncheck it.  This will deselect all the fields.
<img src="snippets/5_map_layer_pop_up_content_display_click.png" width="400">

11. Now click the check box next to the following attributes:
...
12. Under the column `Field Alias`, click on `POP2018`.
13. Change the alias name to `Population 2018`.
14. Click on `SQMI` and change the alias name to `Square Miles`.
...
16. In the `These field attributes will display:` select field, click on `% Frequent Physical Distress..` item to select it.
17. Click on the up arrow to the right of the select field, and move this item to the top.
<img src="snippets/8_map_layer_pop_up_content_move_up.png" width="400">

18. Move the attribute `Life Expectancy...` to the second item on the list.  The result should look like this:
<img src="snippets/9_map_layer_pop_up_content_move_up_final.png" width="400">

19. Click on `OK` button at the bottom to save the changes.
...
1. Hover the mouse over the layer `% Frequent Physical Distress`.
2. Click on the three dots `...` and then `Create Labels`.
<img src="snippets/11_map_layer_create_label_click.png" width="400">

3. In the new form, for the `Text:` field select `COUNTY` from the drop down menu.
...
4. In the map viewer, zoom and and out, and watch the black arrow move.
5. On the left side of the range, click on the `World` field to open a drop down menu.
<img src="snippets/15_map_layer_create_label_world_click.png" width="400">

6. For the `World` input field, type in `1:1,200,000` and hit `Enter` key on your keyboard.
...
7. To make the labels more visible, click on the `Halo` check box and make the font size `14`.
<img src="snippets/18_map_layer_create_label_font_change.png" width="400">
8. Click `OK` at the bottom of the form.


**The labels are now customized.**


## Symbolize Life Expectancy
...
2. In `Content` toggle off the visibility of the `% Frequent Physical Distress` layer.
3. Click on `Add` --> `Search For Layers`
4. Make sure the `My Groups` option is selected at the top.
5. Find the layer `USA Demographics` and click the `+` symbol to add this layer.
6. Click the back arrow button at the top to return to the `Content` pane.
...
7. Hover the mouse over the `USA Demographics` layer and click on `Change Style` button.
8. For `Choose an attribute to show` drop down menu choose `Life Expectancy`.
...
9. Click on the `OPTIONS` button under the category `Counts and Amounts (Color)`.
<img src="snippets/20_map_second_layer_style.png" width="200">
10. In the new pane, click on `Symbols` on the right side.
<img src="snippets/21_map_second_layer_style_color.png" width="300">

11. For the `FILL` select the green to white color ramp that is located in the second row and second column.
<img src="snippets/22_map_second_layer_style_color_pick.png" width="300">

12. Then click the invert button that looks like a circle of arrows.
<img src="snippets/23_map_second_layer_style_color_invert.png" width="300">

13. Click `OK`
14. Click `OK` again to save the changes.
<img src="snippets/24_map_second_layer_style_color_selected.png" width="300">

15. Click `DONE` button at the bottom to get back to the `Content` pane.
...
2. Click on the three dots `...` and then `Rename`.
3. Enter the name as `Life Expectancy` and click `OK`
...
2. Hover the mouse over `% Frequent Physical Distress` layer.
3. Click on the three dots `...` and `Move Up`.
...
2. Click on the three dots `...` and click on `Remove Pop-up`.
...
3. Toggle off the visibility of the `Life Expectancy` layer.
4. Save the map.
...
1. Log onto ArcGIS Online.
2. Click on `Content` and then `My Content`.
3. Click on `Create` button located near the top left, to the right of `Add Item`.
...
4. Click on `Web AppBuilder`.
<img src="snippets/28_my_content_create_web_appbuilder.png" width="700">

5. Fill out the form with the following entries:
* `2D`
...
7. Left pane, click on the `Dashboard Theme`.
<img src="snippets/30_appbuilder_click_dashboard_theme.png" width="400">

8. For `Layout`, select the middle one with 3 widgets on the bottom.
...
9. On the left pane, at the top, click on the `Map` tab.
...
10. Click on `Choose web map` button.
...
11. Select the web map you created, `Web Map: Demographics - <username>`.
12. Click `OK`
...
13. Click on `Save` button located at the bottom of the left pane.
<img src="snippets/40_appbuilder_save.png" width="300">

## Adding Widgets
**Now we will add custom widgets below the map viewer in our applications.**
...
**We are going to add the Layer List widget which will allows users to turn on and off layers in the web map.**
...
3. Click on the `Layer List` widget.
...
5. Uncheck `Show Legend`
6. Uncheck all the options for the `layer context menu`.
...
8. In the left window pane, cover over the newly added `Layer List` widget to see additional options appear.
...
9. Click `Save` at the bottom of the left pane.

### Add Group Filter Widget
...
2. Click on the `Group Filter` widget.
...
5. For the section `Perform a case sensitive search` section, click on `Add Layer`.
<img src="snippets/51_appbuilder_widget_2_group_filter_add_layer.png" width="700">

**This adds another row to the table.**

6. For the second row entery, under column `Layers`, select `% Frequent Physical Distress` layer.
7. Under the `Fields` column, select `STATE` for both layers, as this is the attribute we want to use to filter the data.
8. For `Life Expectancy` layer row, under column `Use Value`, click the radio button.
<img src="snippets/52_appbuilder_widget_2_group_filter_mid_form.png" width="700">

9. At the bottom of the form, make sure the following options have a checkmark
...
11. Select a `AL` state from the drop down menu and then click `Apply` to apply the filter.
...
12. Hit the `Reset` button to remove the filter.
13. Click the `Save` button at the bottom of the left pane.
...
6. At the top, replace `Infographic` with the name of the layer, `% Frequent Physical Distress`.  This will be the tab name.
<img src="snippets/61_appbuilder_widget_3_tab_title.png.png" width="500">

7. Further down the form, click on the top of the preview window, where it says `TITLE OF THE INFOGRAPHIC`.
...
8. Update the `Text` field to `Top 10 Percentage Values`.
<img src="snippets/63_appbuilder_widget_3_title_change.png" width="500">

9. Click the middle of the preview window, where it says `No Data`.
...
10. Under the `Data` tab, have the form contain the following values:
...
11. To the right of `Sort By` field make sure the sort descending (down arrow) is highlighted in blue.
...
13. Use our group filter widget and filter the data for `AL`. 
...
15. Click the `Save` button at the bottom left pane.

**Lets add another plot for Life Expectancy, but we will add it to the same widget pane as out previous plot.**

16. In the left pane, hover over the `Infographic` widget and click on the `+` option.
<img src="snippets/68_appbuilder_widget_3_2_plot.png" width="300">

17. Repeat the steps above for generating a second plot, except, select the `Life Expectancy` as the data source and update the appropriate field names.
...
18. Click the `Save` button at the bottom left pane.

### Add Map Viewer Widgets
...
2. Click on `Legend` and then click `OK`.
<img src="snippets/73_appbuilder_map_widgets_1_legend.png" width="100">

3. Leave everything as default in the configuration window and click `OK`.
...
4. Click on the next available widget space in the left bottom pane.
5. Select the `Swipe` widget.
<img src="snippets/75_appbuilder_map_widgets_2_swipte.png" width="100">

6. Click `OK`
7. Leave the default configuration options and click `OK`.

**The swipe tool allows one to easily compare two layers.  Follow the next steps to use the tool.**

8. Make sure both layers are toggled to be visible in the `Layer List` widget.
<img src="snippets/76_appbuilder_both_layer_visible.png" width="300">

9. Click on the `Swipe` widget in the map viewer.
<img src="snippets/77_appbuilder_swipe_tool_click.png" width="300">

10. A swipe bar will appear in the middle of the map viewer.  Click and drag the swipe bar left and right to compare the layers.
<img src="snippets/78_appbuilder_swipe_bar.png" width="500">

11. Click `Save` at the bottom of the left pane.
...
2. Click on the dashboard item that we just created, `Dashboard: USA Demographics - <username>`
<img src="snippets/81_my_content_click_on_dashboard.png" width="700">
3. Click on the `Share` button on the right side.
<img src="snippets/82_my_content_click_on_share.png" width="700">
4. Click on the appropriate sharing level you want.

**You can also share your application with the RCS Bootcamp group you joined.**

5. Click on `Edit group sharing`.
<img src="snippets/83_my_content_group_share.png" width="500">

6. Select `RCS Bootcamp 2021` group and click `OK`
...
<img src="snippets/85_my_content_share_update.png" width="700">

8. Scroll down the page until the bottom.
9. On the right side, there is a `URL` field.  This is the url you can email to people that you want to share the map with.
<img src="snippets/87_my_content_URL.png" width="700">

10. Scroll back up to the top.
11. To view the application click on `View Application` button in top right.
